...
from src.evaluation.metrics.evaluate_fairness import prepare_compas_dataset, prepare_adult_dataset, compute_fairness_metrics
...
def evaluate_fairness_func(df_real, dataframes, config, save=True):
    ...
    plot2, name_plot2 = plots_fairness.compute_mean_differences_icldemonstration_line(df_fairness, config)
    plot3, name_plot3 = plots_fairness.plot_gender_trends_by_icl(df_fairness, config)

    plots = [plot2, plot3]
    name_plots = [name_plot2, name_plot3]
    ...
    if save:
        ...
        for name_plot, plot in zip(name_plots, plots):
            plot_path = os.path.join(plots_path, f"{name_plot}_fairness_{DATABASE}.pdf")
...
def evaluate_quality_func(df_real, dataframes, config, save=True):
    ...
    for idx, df in enumerate(dataframes):
        task = config["general"]["task"]
        if task == "adult":
            metrics_datasets.append((idx, experiment,  config["dataframes"][idx], evaluate_dataset_models(df_real, df, config, config["dataframes"][idx], save)))
        else:
            metrics_datasets.append((idx, experiment,  config["dataframes"][idx], evaluate_dataset_models(df_real, df, config, config["dataframes"][idx], save)))

    plot, name_plot = plots_quality.plot_evaluation_results_icl_demonstration(metrics_datasets, real_baselines)
    ...
    if save:
        ...
        base_save_path = os.path.join(LOCAL_DIR, FIGURES_PATH)
        df_path = os.path.join(base_save_path, "dataframes")
        plots_path = os.path.join(base_save_path, config["general"]["prompt_id"], "plots")
...
def main():
    ...
    parser.add_argument(
        "--config-path",
        type=str,
        default="./src/configs/evaluation/bias_baseline/eval_icl_demonstration.yaml",
    ...
    # evaluate_quality_func(df_real, dataframes, config, args.save)
